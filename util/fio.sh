#!/bin/bash
# This script can be used to run several fio jobs on a k8s cluster in parallel or sequential mode.
...
while getopts "n:csh" ARGOPTS ; do
    case ${ARGOPTS} in
        c) mode=clean
            ;;
        s) wait=1
...
EOF
  if [[ "${wait}" != "0" ]]; then
    ...
    kubectl wait --for=condition=complete job/fio-test-${uid} -n ${ns} --timeout=10m
  fi
